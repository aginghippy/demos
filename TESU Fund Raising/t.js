var GAM_breakpoint = window.getComputedStyle(document.querySelector('body'), ':before').getPropertyValue('content').replace(/"/g, '');
window.googletag = window.googletag || {cmd: []};
googletag.cmd.push(function () {
  googletag.pubads().setTargeting('sitepage', ['NTB/Home']).setTargeting('site', ["NTB"]).setTargeting('menu_route', ["tb"]).setTargeting('menu_topics', [""]).setTargeting('menu_tags', [""]);
  googletag.pubads().enableSingleRequest();
  googletag.pubads().collapseEmptyDivs();
  googletag.pubads().setCentering(!0);
  googletag.pubads().setForceSafeFrame(!1);
  googletag.enableServices()
});
$(function () {
  function ntb_google_ad_manager_build(item) {
    item_div = $('#' + item.div_id);
    if (item.display_sizes.includes(GAM_breakpoint)) {
      googletag.cmd.push(function () {
        var item = this;
        var item_div = $('#' + item.div_id);
        size_mapping = [];
        if (item.size_mapping.length) {
          size_mapping = googletag.sizeMapping();
          item.size_mapping.forEach(function (size_map) {
            size_mapping.addSize(size_map.view, size_map.map)
          });
          size_mapping.build()
        }
        var targetSlot = googletag.defineSlot(item.slot, item.slot_ad_sizes, item.div_id).setTargeting('pos', [item.target_pos]).defineSizeMapping(size_mapping);
        targetSlot.addService(googletag.pubads());
        googletag.pubads().addEventListener('slotRenderEnded', function (event) {
          if (event.slot === targetSlot && event.isEmpty) {
            item_div.parents('.ad-position').addClass('d-none');
  ...
  function ntb_google_ad_manager_display(item) {
    if (item.display_all || item.display_sizes.includes(GAM_breakpoint)) {
      googletag.cmd.push(function () {
        item = this;
        googletag.display(item.div_id)
      }.bind(item))
    }
  }

  for (let i = 0; i < ntb_google_ad_manager_items.length; i++) {
    ntb_google_ad_manager_build(ntb_google_ad_manager_items[i])
  }
  for (let i = 0; i < ntb_google_ad_manager_items.length; i++) {
    ntb_google_ad_manager_display(ntb_google_ad_manager_items[i])
  }
});
var bxs_update;
var bxs_config = [];
...
bxs_create = function (a, c) {
  var d = "bxs-" + a;
  var b = bxs_breakpoint();
  bxs_config[a] = {
    id: d,
    bp: b,
    swaps: c,
    settings: {
      ...
      pause: 5000,
      pager: !1,
      minSlides: c[b].slides,
      maxSlides: c[b].slides,
      moveSlides: c[b].slides,
      slideMargin: c[b].margin,
      slideWidth: c[b].width,
      nextSelector: "#" + d + "-next",
      prevSelector: "#" + d + "-prev",
      nextText: '<i class="fas fa-2x fa-angle-right"></i>',
      prevText: '<i class="fas fa-2x fa-angle-left"></i>',
      randomStart: !0,
      adaptiveHeight: !0,
      shrinkItems: !0,
      touchEnabled: !1
...
$(function () {
  bxs_activate = function (a) {
    bxs_config[a].bxslider = $("#" + bxs_config[a].id + " .bxslider").bxSlider(bxs_config[a].settings);
    bxs_update(a);
    $(window).resize(function () {
      bxs_update(a)
    })
  };
  bxs_update = function (a) {
    var c = bxs_breakpoint();
    if (c !== bxs_config[a].bp) {
      bxs_config[a].bp = c;
      var b = bxs_config[a].swaps;
      bxs_config[a].settings.minSlides = b[c].slides;
      bxs_config[a].settings.maxSlides = b[c].slides;
      bxs_config[a].settings.moveSlides = b[c].slides;
      bxs_config[a].settings.slideMargin = b[c].margin;
      bxs_config[a].settings.slideWidth = b[c].width;
      bxs_config[a].bxslider.reloadSlider(bxs_config[a].settings)
    }
    $("." + bxs_config[a].id).removeClass("hidden");
    bxs_config[a].bxslider.redrawSlider()
  }
});
Joomla = window.Joomla || {}, Joomla.editors = Joomla.editors || {}, Joomla.editors.instances = Joomla.editors.instances || {}, function (e, t) {
  ...
  e.submitform = function (e, o, n) {
    ...
    strings: {}, _: function (t, o) {
      var n = e.getOptions("joomla.jtext");
      return n && (this.load(n), e.loadOptions({"joomla.jtext": null})), o = void 0 === o ? "" : o, t = t.toUpperCase(), void 0 !== this.strings[t] ? this.strings[t] : o
    }, load: function (e) {
    ...
    if (void 0 === o && (o = t.getElementById("adminForm")), o.boxchecked.value = e ? parseInt(o.boxchecked.value) + 1 : parseInt(o.boxchecked.value) - 1, o.elements["checkall-toggle"]) {
      var n, r, a, i = !0;
      for (n = 0, a = o.elements.length; n < a; n++) if ("checkbox" == (r = o.elements[n]).type && "checkall-toggle" != r.name && !r.checked) {
        i = !1;
        break
      }
    ...
    var u, m, p, h = n == r;
    for (u = 0; u < o.length; u++) if ((p = o[u])[0] == n) {
      m = h ? a == p[1] : 0 === u;
    ...
    if (o = o || "show", n = n || t.body, "load" === o) {
      var r = (e.getOptions("system.paths") || {}).root || "", a = t.createElement("div");
    ...
    try {
      ...
      if (t.headers) for (var r in t.headers) t.headers.hasOwnProperty(r) && o.setRequestHeader(r, t.headers[r]);
...
!function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    var o = Joomla.getOptions("system.keepalive"), n = o && o.uri ? o.uri.replace(/&amp;/g, "&") : "",
...
window.addEventListener("load", function () {
  window.cookieconsent.initialise({
    "palette": {
      "popup": {"background": "#293c7b", "text": "#ffffff"},
      "button": {"background": "#f8f9fa", "text": "#212529"}
    },
    "theme": "classic",
    "content": {
      "message": "Tech Briefs Media Group websites place cookies on your device to give you the best user experience. By using our websites, you agree to the placement of these cookies. To learn more, read our ",
...
$(function () {
  olytics.fire({behaviorId: '5013J3562790A3O', category: "Home", tag: "", pageType: "index", contentType: "topic",})
});
var match_md_max = window.matchMedia("(max-width: 991px)");
bxs_create('default', {
  xl: {slides: 5, margin: 20, width: 256},
  lg: {slides: 4, margin: 32, width: 256},
  md: {slides: 3, margin: 35, width: 256},
  sm: {slides: 3, margin: 20, width: 256},
  xs: {slides: 2, margin: 40, width: 256}
});
$(function () {
  bxs_activate('default')
});
$(document).ready(function () {
  mh = function (mq) {
    if (mq.matches) {
      $('#f2_36756>a').appendTo('#f1_36756')
    } else {
      $('#f1_36756>a').appendTo('#f2_36756')
    }
  };
  match_md_max.addListener(mh);
  mh(match_md_max)
});
$(document).ready(function () {
  mh = function (mq) {
    if (mq.matches) {
      $('#f2_36622>a').appendTo('#f1_36622')
    } else {
      $('#f1_36622>a').appendTo('#f2_36622')
    }
  };
  match_md_max.addListener(mh);
  mh(match_md_max)
...
$(document).ready(function () {
  mh = function (mq) {
    if (mq.matches) {
      $('#f2_36624>a').appendTo('#f1_36624')
    } else {
      $('#f1_36624>a').appendTo('#f2_36624')
    }
  };
  match_md_max.addListener(mh);
  mh(match_md_max)
...
$(document).ready(function () {
  mh = function (mq) {
    if (mq.matches) {
      $('#f2_36623>a').appendTo('#f1_36623')
    } else {
      $('#f1_36623>a').appendTo('#f2_36623')
    }
  };
  match_md_max.addListener(mh);
  mh(match_md_max)
...
$(document).ready(function () {
  mh = function (mq) {
    if (mq.matches) {
      $('#f2_36621>a').appendTo('#f1_36621')
    } else {
      $('#f1_36621>a').appendTo('#f2_36621')
    }
  };
  match_md_max.addListener(mh);
  mh(match_md_max)
...
$(document).ready(function () {
  mh = function (mq) {
    if (mq.matches) {
      $('#f2_36619>a').appendTo('#f1_36619')
    } else {
      $('#f1_36619>a').appendTo('#f2_36619')
    }
  };
  match_md_max.addListener(mh);
  mh(match_md_max)
...
$(document).ready(function () {
  mh = function (mq) {
    if (mq.matches) {
      $('#f2_36369>a').appendTo('#f1_36369')
    } else {
      $('#f1_36369>a').appendTo('#f2_36369')
    }
  };
  match_md_max.addListener(mh);
  mh(match_md_max)
...
$(document).ready(function () {
  mh = function (mq) {
    if (mq.matches) {
      $('#f2_36377>a').appendTo('#f1_36377')
    } else {
      $('#f1_36377>a').appendTo('#f2_36377')
    }
  };
  match_md_max.addListener(mh);
  mh(match_md_max)
...
$(document).ready(function () {
  mh = function (mq) {
    if (mq.matches) {
      $('#f2_36618>a').appendTo('#f1_36618')
    } else {
      $('#f1_36618>a').appendTo('#f2_36618')
    }
  };
  match_md_max.addListener(mh);
  mh(match_md_max)
...
$(document).ready(function () {
  mh = function (mq) {
    if (mq.matches) {
      $('#f2_36364>a').appendTo('#f1_36364')
    } else {
      $('#f1_36364>a').appendTo('#f2_36364')
    }
  };
  match_md_max.addListener(mh);
  mh(match_md_max)
});
bxs_create('tbtvrelated', {
  xl: {slides: 4, margin: 20, width: 224},
  lg: {slides: 3, margin: 10, width: 260},
  md: {slides: 3, margin: 5, width: 200},
  sm: {slides: 2, margin: 30, width: 215},
  xs: {slides: 2, margin: 2, width: 275}
});
$(function () {
  bxs_activate('tbtvrelated')
});
$(function () {
...
$(window).scroll(function () {
  if ($(document).scrollTop() > 50) {
    $("nav").addClass("shrink");
    $(".logolarge").addClass("tb-logo-closed");
    $(".logosmall").removeClass("d-md-none");
    $(".add").hide()
  } else {
    $("nav").removeClass("shrink");
    $(".logosmall").addClass("d-md-none");
    $(".logolarge").removeClass("tb-logo-closed");
    $(".add").show()
  }
});
$(document).ready(function () {
  function b() {
    $("#navbarSideContainer").addClass("reveal");
    $(".overlay").show()
  }

  function a() {
    $("#navbarSideContainer").removeClass("reveal");
    $(".overlay").hide()
  }

  $("[id*=navbar-side-button]").on("click", function () {
    b()
  });
  $("[id*=search-side-button]").on("click", function () {
    b();
    $("#searchbox").focus()
  });
...
function checkWidth() {
  if ($(window).width() >= 768) {
    $(".tb-filter-show").addClass("show")
  } else {
    $(".tb-filter-show").removeClass("show")
  }
}
